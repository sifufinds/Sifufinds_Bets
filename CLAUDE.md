...
| Title/URL/body claims a topic the content never delivers — specifically, a "transfers"/"football" post whose title, slug, OR the body's own prose (opening/closing sentences, not just metadata) frames a story as African ("Transfer Frenzy in Africa", "Africa's Top Transfer Stories", "African clubs are making some exciting moves") when the body names zero real African country, league, or competition — i.e. it's actually European/global club news mislabelled as African (found 2026-08-16 — user-reported live example: `/blog/transfer-frenzy-in-africa-.../` was entirely about West Ham, Sunderland, Chelsea and Newcastle; a same-day repo-wide audit found 7 more live instances, some from the original LLM generation and some stale slugs left over from an earlier fact-check correction pass that fixed the title/body but never renamed the URL; a same-day **follow-up** report found the false framing also survived inside the body prose of already-"fixed" posts — the original check only ever looked at `title`/`slug`, never `body`) | `agents/python/utils/title_content_match.py`'s `check_africa_framing()` — deterministic (not LLM-based: the failure is a checkable fact, not a judgement call) regex+word-list check requiring BOTH an explicit "story is African" claim (title, slug, OR any body sentence — checked sentence-by-sentence so the proximity regex can't bridge two unrelated sentences) AND a genuine absence of any real African country name, demonym ("Nigerian striker"), league, or competition anywhere in the body. The claim regex allows at most one filler word between "africa(n)/africa's" and the subject noun in either direction (catches "Transfer Frenzy in Africa" and "African top clubs"; rejects "African bettors follow the transfer market" — legitimate audience framing, not a subject claim). Subject-noun list is deliberately narrow (`transfer|club|star|footballer|squad|deal|window`) — `team`/`league`/`player` were tried and reverted after producing false positives on legitimate World Cup coverage ("African teams have exited the tournament" is true) and betting copy ("African players" = bettors, not footballers, in gambling-site prose). Wired into `agent_sports_blog.py`'s `generate_post()` as a new gate alongside the existing fact-checker — a violation holds the draft back the same way a fact-check FLAG does (nothing publishable this run, not a crash). `SYSTEM_PROMPT` also hardened with an explicit non-negotiable rule + WRONG/RIGHT example matching this exact incident. Verified 0 false positives/negatives against all 875 live posts. The 8 posts found in the title/slug audit were retitled/re-slugged with 301 redirects added to `.htaccess`; the 3 posts found with residual false framing still in their body prose (2 from the original 8, 1 more found only by the body-level scan) had those specific sentences corrected in place | `scripts/seo_check.py` section 4c (pre-deploy gate, blocking, scans every post's title/slug/body in `blog/posts.json` — catches a violation reaching posts.json through ANY path, not just `agent_sports_blog.py`, e.g. Sanity CMS sync or a manual edit) + `agent_sports_blog.py`'s generation-time gate on every run |

| Truncated titles/excerpts ending mid-thought on a dangling conjunction/article/preposition/possessive ("...Man Utd's Rebuild, and", "...Betting Tips Kenya: Best") or a bare trailing ellipsis even when the preceding word was fine ("...Comprehensive Comparison…") — plus the same defect intermittently colliding two different translated posts onto one identical `<title>` tag and blocking the daily Fabricated Content Fix workflow's commit (found 2026-08-21, user-reported as both "blog posts keep failing" and "grammar mistakes in the titles"; root-caused to two separate truncation implementations — `seo_meta.py`'s `seo_title()` kept only the text *before* a `": "` separator, which for this site's own "Tournament Year: match detail" title shape discards the one part that makes two posts different, and `scripts/seo_check.py --fix`'s own cruder `original[:60].rsplit(' ', 1)[0]` mutated `post['title']`/`post['excerpt']` in posts.json directly with no check for whether the cut landed on a complete phrase) | `seo_meta.py`'s `seo_title()` no longer special-cases separator-prefix truncation (pure tail-based word-boundary cut instead, which can't collapse two titles onto one shared generic prefix); new `strip_dangling_words()`/`ends_with_dangling_word()` drop a trailing run of function words/possessives/leftover punctuation, with a small `_SAFE_TRAILING_PHRASES` exception ("talks on", "rolls on", ...) for this site's own idiomatic transfer-headline style ("Savinho Talks On") so the fix doesn't re-break a title that was already complete. `scripts/seo_check.py --fix` and `scripts/audit_titles.py` (which had its own third, independently-drifted copy of the old logic) both now route through the same `seo_meta.py` functions instead of hand-rolling their own. Retroactively repaired 790 already-broken fields across `blog/posts.json` (243+26+1) and 4 of 5 `blog/translations/*.json` files (74 entries) down to 0 issues | `scripts/seo_check.py` section 2b, `dangling_title`/`dangling_excerpt` (HIGH, wired into `--fix`) — pre-deploy gate + daily auto-heal + every generator's own run |
...
## STANDING RULE — RapidWombat Webhook Is an Authoring Source (added 2026-09-26)

**RapidWombat articles reach the site only through `blog/posts.json`, the same as the Sanity sync.** Endpoint: `https://sifufinds.com/webhooks/rapidwombat.php` (Bearer-token auth) → GitHub `repository_dispatch` → `.github/workflows/rapidwombat_import.yml` → `agents/python/agent_rapidwombat_import.py` → `gen_blog_post_pages.py` → all 6 pre-deploy gates → commit. Every blog standing rule applies unchanged; nothing is ever written on the Hostinger server.
- Secrets: `RAPIDWOMBAT_WEBHOOK_TOKEN` (the token pasted into RapidWombat) and `RAPIDWOMBAT_DISPATCH_TOKEN` (fine-grained PAT, Contents read/write on this repo) are GitHub repo secrets. `deploy_hostinger.yml` writes them into `webhooks/rapidwombat-secrets.php` in the deploy copy only. Never commit that file (the repo is public).
- Do not add `rapidwombat_import.yml` to `workflow_watchdog.yml`: a `workflow_dispatch` retry has no article payload. The workflow retries itself by re-dispatching its own payload.
- Test the endpoint: open the URL in a browser, which should return `"configured": true`. Tests: `python3 agents/python/test_rapidwombat_import.py`.
...
- `scripts/hide_brands.py --apply` runs in `deploy_hostinger.yml` (step "Hide hidden brands from the deploy copy", after the validators, before the archive) on the CI checkout only. It: turns every page *about* a hidden brand (path under `bookmakers/<brand>/`, or `<title>`/`<h1>` naming one) into a noindex redirect stub + adds a `RedirectMatch 301` to the deploy copy's `.htaccess` + drops it from sitemaps; strips brand cards/ads, table columns/rows, FAQ items, heading sections, tag chips, links, list items and sentences from every other page (`scripts/hidden_brands_html.py`); scrubs JSON data files, `window.X={...}` data scripts, JS code (one-line object literals such as `BOOKS` entries are dropped, brand map pairs/array items removed, prose strings and comments cleaned, and `shared.js`'s own `HIDDEN_BRAND_*` lists shipped base64-encoded), JSON-LD, `llms.txt`; then re-scans everything and **fails the deploy if any mention remains anywhere in a deployed text file** — visible text, attributes, inline scripts, comments, JS, JSON, XML. It also deletes `data/hidden_brands.json` from the deploy copy so the list itself isn't published.
