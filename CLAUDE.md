...
|---|---|---|
| CMO Agent | ⚠️ Exists but **dormant** — not wired into any GitHub Actions workflow | `agents/python/agent5_plan.py` ("Growth Planner" — weekly editorial/social plan from content+SEO+queue state). Needs a workflow (`agent5_plan.yml`) before it's actually running, not just a code fix |
...
| Keyword Research Agent | ❌ Missing | No standing agent. `agents/python/utils/serp_research.py` does free-first search but only per-post at write time, no persisted keyword-opportunity backlog |
...
| Sportsbook Review Writer | ⚠️ Covered but fragmented | `agent_brand_reviews.py` + `agent_brand_discovery.py` — see brand-data duplication note below |
...
| Bonus Content Writer | ❌ Missing | `gen_bonus_pages.py` is static/hand-authored, no writer agent |
...
| SERP Monitoring Agent | ❌ Missing | No historical rank-tracking storage exists anywhere; `serp_research.py`'s free search is the only reusable building block |
