...
## STANDING RULE — AI Agent Team Roadmap (added 2026-07-30)

The business side of SifuFinds maps to a target org chart (Strategy / SEO & GEO / Content / Product & UX / Data & Monitoring / Technical roles, plus Featured Listings monetization). Phase 1 of building that team out landed 2026-07-30: news→reviews/bonus/comparison internal linking (`gen_blog_post_pages.py`'s `BONUS_LINKS`/`COMPARISON_LINKS`/`_pick_news_context_links()`), the Compliance Reviewer (`scripts/compliance_check.py`), the Fact Checker (`agents/python/agent_fact_checker.py`), a general Country Localisation agent (`agents/python/agent_translate.py`, generalised from an initial European-Portuguese-only design), and Featured Listings (`data/featured_listings.json` + `agent_brand_partnerships.py --manage-listings` + the sponsored badge in `bookmaker_page_template.py`).

**Several org-chart roles were already substantially covered before Phase 1** — don't rebuild these: Technical SEO ≈ `scripts/seo_doctor.py`/`check_indexability.py`, Commercial Partnerships (outreach side) ≈ `agent_brand_partnerships.py`'s existing email flow, Broken Link & QA ≈ `scripts/validate_site.py` + `agents/python/site_doctor.py`.

**Phase 2 (next)**:
- Casino Review Writer / Bonus Content Writer / Betting Guide Writer — blocked on consolidating brand-fact data first: `gen_bk_reviews.py`, `agent_brand_reviews.py`, and `agent_telegram_offers.py`'s `BRANDS` list each hardcode their own overlapping schema for the same brands, with `data/bookmaker_links.json` the only real source of truth (and it only tracks link/status metadata, not review copy). Build these writers against one canonical `data/bookmakers.json`, not a fourth hardcoded list.
- Keyword Research Agent — reuse `agents/python/utils/serp_research.py`'s free-first search plumbing as a building block; write findings to a keyword-opportunity backlog `agent_sports_blog.py`/`agent_content_backfill.py` can draw from.
- GEO Optimisation Agent — audit existing pages against the GEO rules already documented above (direct-answer opening, extractable FAQ passages, named-entity precision) and feed gaps into `agent_content_backfill.py`'s existing queue rather than building a separate backfill mechanism.
- Affiliate Offer Monitoring Agent — checks that bonus amounts/terms shown across `bonuses/`, `bookmakers/`, `data/countries_live.json`, `data/bookmaker_links.json` are still accurate against live operator pages, reusing `agents/python/utils/free_scrape.py`. Closest existing analogue is `agent_brand_discovery.py`'s `verify_existing_brands()`, which only checks liveness, not offer accuracy — real gap.
- SERP Monitoring Agent — reuse `serp_research.py`'s free DuckDuckGo search; track keyword rank position over time in a JSON state file (no historical ranking storage exists anywhere yet).

**Phase 3 (later)**: Competitor Monitoring Agent; off-page/visual-regression extensions to Broken Link & QA (`validate_site.py`/`site_doctor.py` only do static analysis + live-HTTP checks today, no cross-site or visual checks); Performance & Core Web Vitals Agent (no Lighthouse/PageSpeed automation exists yet); UX Optimisation / CRO / Landing Page Optimisation agents (need GA4 data wired in first — see the 2026-07-26 GA4/Search Console entry above); a CMO strategy agent synthesising the others' output into a weekly memo; locale expansion beyond `pt` to `es`/`sw` (same `agent_translate.py`, just point `--locale` at them) and to non-blog pages (country/bonus/guide pages have zero translation infrastructure today, unlike blog posts).

**Not standing agents**: Front-end Developer Agent / Back-end Developer Agent roles from the org chart map to ad hoc Claude Code sessions working directly in this repo, not a cron job to build — no script needed for these two.

**Featured Listings rendering scope note**: the sponsored badge currently only renders on `bookmakers/<slug>/index.html` pages generated via `bookmaker_page_template.py` (a pure function, safe to extend and test in isolation). Broader placements (`homepage_hero`, listing tables on `index.html`/`countries/*/index.html`) are NOT yet wired up — those pages each define their own inline `fetchLiveData()`-style script rather than sharing logic through `assets/shared.js`, and bookmaker cards have no stable `data-*` attribute to hook a badge onto. Wiring those slots safely needs either touching every page's inline script individually (higher regression risk) or first centralizing bookmaker-card rendering into `shared.js` — treat that as its own prerequisite task, not a quick addition.
