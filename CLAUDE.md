...
| Keyword Research Agent | ✅ Built Phase 2 | `agents/python/agent_keyword_research.py` — resumable batches of seed keywords (core topics × countries) via `utils/serp_research.py`'s free search, persists ranking/competitor/PAA-gap findings to `keyword_opportunities.json`, scheduled via `agent_keyword_research.yml` |
