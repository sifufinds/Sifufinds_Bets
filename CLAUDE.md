...
| Keyword Research Agent | ✅ Built Phase 2, extended 2026-08-01 | `agents/python/agent_keyword_research.py` (evergreen money keywords, resumable batches of seed keywords × all 23 countries via `utils/countries.py` — was only 6 countries via `config.COUNTRIES` until 2026-08-01) + `agents/python/agent_trending_keywords.py` (news-driven trending keywords, pairs today's fresh headlines from `utils/news_fetcher.py` with each country's betting-intent modifiers so the backlog stays current instead of static). Both use `utils/serp_research.py`'s free search, persist to `keyword_opportunities.json`/`trending_keywords.json`, scheduled via `agent_keyword_research.yml`/`agent_trending_keywords.yml`. Output is fed into `agent5_plan.py`'s weekly editorial-calendar prompt so real keyword gaps drive blog/news/guide topics instead of the LLM inventing them, **and** (2026-08-02) into `agents/python/agent_content_priority.py`, which ranks every unranked gap by country tier × content type into `content_priority_queue.json` — the queue `agent_priority_writer.py` actually writes from, closing the loop from "gap found" to "post published". Scheduled every 2 hours via `agent_priority_content.yml` |
...
**Phase 2 shipped 2026-07-30**: Keyword Research Agent, SERP Monitoring Agent, and Performance & CWV Agent were all built same-day as Phase 1 — none had a GA4-data or brand-data-consolidation dependency blocking them, matching the priority order this table originally called out. `agent5_plan.py` (CMO Agent) was also wired into a real schedule for the first time. Casino Review Writer / Bonus Content Writer / Betting Guide Writer remain the next priority but are still blocked on consolidating brand-fact data first: `gen_bk_reviews.py`, `agent_brand_reviews.py`, and `agent_telegram_offers.py`'s `BRANDS` list each hardcode their own overlapping schema for the same brands, with `data/bookmaker_links.json` the only real source of truth (link/status metadata only, no review copy) — build these writers against one canonical `data/bookmakers.json`, not a fourth hardcoded list.
...
**Featured Listings rendering scope note (updated 2026-08-08)**: the sponsored badge renders in two places now. `bookmakers/<slug>/index.html` pages (via `bookmaker_page_template.py`, a pure function) show the "Featured Partner" badge. `countries/<slug>/index.html` pages (all 23) show a 5-card paid "🔥 Top Offers" strip above the hero — see the "Country Sponsor Placements" standing rule below for how it works. Both read the same `data/featured_listings.json` single source of truth. Still NOT wired up: `homepage_hero` on `index.html` and any listing-table row-level badge — `index.html` still defines its own inline script rather than sharing logic through `assets/shared.js`, and bookmaker cards there have no stable `data-*` attribute to hook a badge onto. Wiring that specific slot safely still needs either touching `index.html`'s inline script directly or first centralizing its bookmaker-card rendering into `shared.js`.

## STANDING RULE — Country Sponsor Placements (added 2026-08-08)

Every `countries/<slug>/index.html` page (all 23) renders a 5-card paid-placement strip ("🔥 Top Offers") at the very top of the page, above the H1 hero and below the site-wide "🔥 Featured" header bar — separate from the existing organic "⭐ Top Picks" grid further down the page (`renderFeatCards()`, now capped at 6 cards, was 8). Brands pay SifuFinds a flat monthly fee to hold a slot here.

- **Mechanism**: `assets/shared.js`'s `renderSponsorStrip()` (self-triggered on `DOMContentLoaded`, no per-page wiring needed beyond the `<div id="sponsor-strip" class="sponsor-strip"></div>` placeholder already present in every country page and in `generate_country_pages.py`'s template) fetches `data/featured_listings.json` client-side and looks for active entries whose `slot` matches `country_sponsor:<country-slug>:<position>` (position 1-5, `<country-slug>` = the page's own directory name, e.g. `nigeria`). Managed the same way as the existing bookmaker-review "Featured Partner" badge: `agents/python/agent_brand_partnerships.py --add-listing --slot country_sponsor:nigeria:1 --brand-slug melbet --ends <ISO8601> --criteria "<reason>"`.
- **Eligibility gate (hard requirement, per the user's explicit ask)**: a paid slot only ever renders as "Sponsored" if `brand_slug` resolves to a live `BOOKS[<cty>]` entry whose `url` matches one of the 9 documented tracking domains in `assets/shared.js` `AFFILIATE_DOMAINS` (kept in sync with the "Brands With a Real Affiliate Link" rule above — this array was actually missing 3 of the 9 documented domains until this same date, now fixed). A brand without a real affiliate link can never occupy a slot, sold or not — the render function silently skips a non-matching paid entry rather than showing it.
- **Never-empty fallback**: any unsold or non-matching position (today: all of them, zero slots sold anywhere) is filled with that country's next top affiliate-linked `BOOKS` entry, unlabeled (no "Sponsored" tag) — so the strip always looks fully populated rather than showing a "buy this slot" gap, without ever mislabeling an unpaid pick as sponsored. Countries with fewer than 5 real-affiliate-linked brands (Ethiopia, DR Congo) simply render fewer cards — `.spc-grid` uses `auto-fit` with a capped max width so 1-2 cards don't stretch into an ugly full-width empty strip.
- **Compliance**: `scripts/compliance_check.py`'s existing `check_featured_listings_labels()` already covers `country_sponsor:` entries generically (requires `criteria_note` on every active listing) — no separate check was needed.
- **If you touch this**: `_pathRoot()` in `shared.js` (used by `renderSponsorStrip()`'s fetch path and the pre-existing "inject About link" nav logic, which had an actual off-by-one bug for 2-level-deep pages like `countries/nigeria/` until this same fix — it was silently generating `../about/` instead of `../../about/`) is the one shared depth-to-root-prefix helper; don't reintroduce a second copy of that calculation. `_brandSlugForBook()` derives a brand's slug from `BRAND_DOMAINS[abbr]`'s first domain label — keep `BRAND_DOMAINS` and `agents/python/utils/affiliate_links.py` `BRAND_SLUGS` pointed at the same canonical slugs or a sold slot will silently fail to match and fall back to organic filler.
